...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateUserProfile = useCallback(async (newName: string, newAvatarDataUrl: string | null): Promise<boolean> => {
    try {
      ...
      if (newName && newName !== user.name) {
          ...
          if (user.accountType === 'creator') {
              ...
              nameExists = mockUserListFromStorage.some(u => u.id !== user.id && u.name === newName) || 
                           mockAuthors.some(author => author.name === newName); 
          }
      ...
      return true;
        ...
        toast({ title: "Profile Update Failed", description: "An unexpected error occurred.", variant: "destructive" });
        return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const purchaseAccess = useCallback(async (mangaId: string, accessType: 'monthly' | 'chapter', itemId: string, price: number): Promise<boolean> => {
    try {
      ...
      console.error("Error during purchaseAccess:", error);
  ...
  const donateToManga = useCallback(async (mangaId: string, mangaTitle: string, authorId: string, amount: number): Promise<boolean> => {
    try {
      ...
      toast({ title: "Donation Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const investInManga = useCallback(async (mangaId: string, mangaTitle: string, sharesToBuy: number, pricePerShare: number, totalCost: number): Promise<boolean> => {
    try {
      ...
      toast({ title: "Investment Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const rateManga = useCallback(async (mangaId: string, score: 1 | 2 | 3): Promise<boolean> => {
    try {
      ...
      toast({ title: "Rating Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  ...
  const addMangaSeries = useCallback(async (
    ...
    try {
      ...
      const authorInfoForManga: GlobalAuthorInfo = { 
          ...
          contactDetails: authorInfo.contactDetails, 
      ...
      console.error("Error adding manga series:", error);
      toast({ title: "Manga Creation Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return null;
    }
  }, [user, toast, recordTransaction]);
  ...
  const deleteMangaSeries = useCallback(async (mangaId: string): Promise<boolean> => {
    try {
      ...
      console.error("Error deleting manga series:", error);
      toast({ title: "Manga Deletion Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const addFunds = useCallback((amount: number) => {
    try {
      ...
      toast({ title: "Failed to Add Funds", description: "An unexpected error occurred.", variant: "destructive" });
    }
  }, [user, toast, recordTransaction]);
  ...
  const withdrawFunds = useCallback(async (amountToWithdraw: number): Promise<boolean> => {
    try {
      ...
      toast({ title: "Withdrawal Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const toggleFavorite = useCallback((mangaId: string, mangaTitle: string) => {
    try {
      ...
      toast({ title: "Favorite Action Failed", description: "An unexpected error occurred.", variant: "destructive" });
    }
  }, [user, toast]);
  ...
  const listSharesForSale = useCallback(async (mangaId: string, shares: number, pricePerShare: number, description: string): Promise<ShareListing | null> => {
    try {
      ...
      toast({ title: "Share Listing Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return null;
    }
  }, [user, toast, recordTransaction]);
  ...
  const delistSharesFromSale = useCallback(async (mangaId: string, listingId: string): Promise<boolean> => {
    try {
      ...
      console.error("Error delisting shares:", error);
      toast({ title: "Share Delisting Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const purchaseSharesFromListing = useCallback(async (listingId: string, sharesToBuy: number): Promise<boolean> => {
    try {
      ...
      toast({ title: "Share Purchase Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return false;
    }
  }, [user, toast, recordTransaction]);
  ...
  const followShareListing = useCallback((listingId: string) => {
    try {
      ...
      toast({ title: "Follow Action Failed", description: "An unexpected error occurred.", variant: "destructive" });
    }
  }, [user, toast]); 
  ...
  const unfollowShareListing = useCallback((listingId: string) => {
    try {
      ...
      toast({ title: "Unfollow Action Failed", description: "An unexpected error occurred.", variant: "destructive" });
    }
  }, [user, toast]); 
  ...
  const addCommentToManga = useCallback(async (mangaId: string, text: string, parentId?: string): Promise<Comment | null> => {
    try {
      ...
      if (user.accountType === 'creator' && currentManga && currentManga.author.id !== user.id) {
        toast({ title: "Action Not Allowed", description: "Creators can only comment on their own works.", variant: "destructive" });
        return null;
      }
      if (!currentManga) {
        toast({ title: "Manga Not Found", variant: "destructive" });
      ...
      if (addedComment) {
          ...
          toast({ title: "Comment Added!", description: `Your ${parentId ? 'reply' : 'comment'} has been posted.` });
          return addedComment;
      } else {
      ...
      toast({ title: "Comment Failed", description: "An unexpected error occurred.", variant: "destructive" });
      return null;
    }
  }, [user, toast, recordTransaction]);
